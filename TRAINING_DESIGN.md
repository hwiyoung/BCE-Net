...
2026-07-22 사후 감사에서 8,192픽셀 block 자체는 split 사이에 공유되지
않지만 block 경계를 가로지르는 1024 patch의 실제 픽셀이 겹침을 확인했다.
v1 manifest는 file-disjoint이지만 strict spatial-disjoint split은 아니다.
근거는 `training_monitor/test-split-overlap-audit-20260722`에 있다. 기존
실행 재현을 위해 v1을 보존하고, 256px buffered component를 사용하는 v2
manifest를 `dataset/map_ortho_manifest_spatial_v2.csv`로 별도 생성했다.
독립 감사에서 v2의 cross-split source 1024, center 512, train possible-crop
union overlap이 모두 0임을 확인했다. 현재 robust baseline의 v2 재학습과
frozen test 결과는 `SPATIAL_V2_BASELINE_REPORT.md`에 기록했다.
